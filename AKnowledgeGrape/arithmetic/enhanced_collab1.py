...
logger = logging.getLogger("EnhancedCollaborativeLearning1")


class EnhancedCollaborativeLearning1:
    ...
    def show_path_node_attributes(self, path):
        """展示路径上每个知识点的所有关键属性"""
        if not path or len(path) == 0:
            logger.warning("无有效路径，无法展示节点属性")
            return

        print("\n=== 路径节点属性详情 ===")
        print(f"{'节点ID':<8} {'标签':<20} {'知识点名称':<30} {'章节ID':<10} {'布鲁姆层级':<12} {'权威值(edu_pr)':<15} {'是否核心知识点':<15}")
        print("-" * 120)
        ...
        for idx, nid in enumerate(path, 1):
            ...
            # 关键属性提取（默认值兜底，避免None报错）
            label = node_info.get('label', '未知标签')
            title = node_info.get('title', '未命名知识点')[:28]  # 截断过长名称，保证格式整齐
            chapter_id = node_info.get('chapter_id', 0)
            bloom_level = node_info.get('bloom_level', 0)
            edu_pr = round(node_attrs.get('edu_pr', 0.0), 6)
            is_core = node_info.get('is_core', False)
            
            # 格式化输出
            print(f"{nid:<8} {label:<20} {title:<30} {chapter_id:<10} {bloom_level:<12} {edu_pr:<15} {str(is_core):<15}")

        # 补充路径属性统计信息
        total_nodes = len(path)
        core_count = sum(1 for nid in path if self.node_info.get(nid, {}).get('is_core', False))
        avg_bloom = np.mean([self.node_info.get(nid, {}).get('bloom_level', 0) for nid in path])
        avg_edu_pr = np.mean([self.node_attrs.get(nid, {}).get('edu_pr', 0.0) for nid in path])

        print("\n=== 路径属性统计 ===")
        print(f"路径总节点数：{total_nodes}")
        print(f"核心知识点数：{core_count}（占比：{core_count/total_nodes:.2%}）")
        print(f"平均布鲁姆认知层级：{avg_bloom:.2f}")
        print(f"平均节点权威值：{avg_edu_pr:.6f}")
    ...
    def load_graph(self):
        ...
        for rec in nodes_result:
            ...
            self.node_info[nid] = {
                ...
                # 缓存bloom_level等后续可能用到的属性（如果节点有该属性）
                'bloom_level': rec.get('bloom_level', 0)
            }
...
if __name__ == "__main__":
    # 定义多组测试用例（不同起止节点组合）
    test_cases = [
        {"start": 594, "goal": 654,"desc": "跨章节长路径"},
        {"start": 461, "goal":389, "desc": "跨章节长路径"},
        {"start": 389, "goal":527, "desc": "跨章节长路径"},
        {"start": 370, "goal":654, "desc": "跨章节长路径"}
    ]
    ...
    for case in test_cases:
        # 每次测试创建新实例
        system = EnhancedCollaborativeLearning1(
        ...
        for res in final_results:
            ...
            print(f"  路径节点名称序列: {[system.get_node_details(nid)['title'] for nid in res['path']] if res['path'] else '无'}")
            print(f"  最终成本: {res['cost']:.4f}")
            # 调用节点属性展示方法
            system.show_path_node_attributes(res['path'])
