...
面向计算机专业大学课程的**教育知识图谱框架**，支持多层知识本体建模、基于改进 Edu-link PageRank 的知识权重评估、认知自适应学习路径生成与闭环动态更新，致力于推动可持续的个性化工程教育。

> 当前数据与案例基于「大数据处理架构 Hadoop」课程构建，方法论可迁移至任意课程。
...
传统课程知识点之间天然存在「先修—后继」依赖关系，但不同知识点对达成教学目标的重要性并不相同。本项目将课程知识体系建模为有向图，并引入**教育语义因子**对经典 PageRank 进行改造，从而得到更能反映教学价值的知识点权重；在此基础上，结合学习者认知水平，生成自适应的个性化学习路径，并通过学习反馈实现图谱权重的闭环更新。

主要技术路线：

1. **多层本体建模**：章节（Chapter）→ 知识点（KnowledgePoint）→ 子知识点（SubKnowledgePoint），并关联实验（Lab）。
2. **改进 Edu-link PageRank**：在标准 PageRank 基础上引入 `class_hours`（课时）、`syllabus_mentions`（大纲提及次数）、`bloom_level`（布鲁姆认知层级）等教育因子。
3. **认知自适应学习路径**：基于增强 Dijkstra / 认知导向 A* 算法，结合知识点权重与学习者能力生成个性化路径。
4. **闭环动态更新**：采集学习反馈，动态调整知识点权重与路径。
...
### 1. 数据层（`Practice/Data/` 与 `D-Data`、`O-Data`）
...
### 5. 闭环动态更新（`arithmetic/` 与 `confirmation/`）
...
> 运行脚本时，请将工作目录设为脚本所在目录（部分脚本依赖相对导入，如 `core_knowledge_validation.py` 导入 `pagerank` / `edu_pagerank`）。
...
其中 `bloom_level` 对应布鲁姆认知层级：1 记忆、2 理解、3 应用、4 分析、5 评价、6 创造。
